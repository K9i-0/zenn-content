...
### ChatGPTのざっくちとした仕組み
ChatGPT APIはモデルとメッセージを指定してAPIを叩きます。

モデルは現状gpt-3.5-turboを設定すれば良いです。
メッセージはロールとコンテンツに別れます。
コンテンツはChatGPTのUIで表示されるテキストです。
ロールはassistant, user, systemの３種類があります。
- assistant ChatGPT
- user 人間
- system 調整用メッセージにつかう
  - あなたはリードエンジニアですなど、会話の前提を設定する

https://platform.openai.com/docs/api-reference/chat

### dart_openaiでChatGPT APIを叩く
OpenAI.instance.chat.createというメソッドを叩けばよいです。
このときmesseagesにはこれまでのやり時も含めて指定します。
```dart
...
### チャットUI作ってみた
riverpodを使って、ChatGPTとやりとりしたメッセージのリストを状態にもち、新しいメッセージを送れるProviderを作ると以下のようになります。
...
今回はかなりシンプルな例でしたが、工夫次第で夢が広がりそうですね。

今回のサンプルを実装したリポジトリを公開しているので、よかったら使ってみてください。
