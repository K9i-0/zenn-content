...
以下のように書くとProviderが生成されます。（正確にはAutoDisposeProvider<String>）
```dart
// hogeProviderを作る場合
@riverpod
...
以下のように書くとFutureProviderが生成されます。（正確にはAutoDisposeFutureProvider<String>）
```dart
// hogeProviderを作る場合
@riverpod
...
_${クラス名}を継承したクラスを作り、Riverpodアノテーションをつけ、buildメソッドをoverrideするとNotifierProviderになります。
以下のように書くとNotifierProviderが生成されます。（正確にはAutoDisposeNotifierProvider<Hoge, HogeState>）
```dart
class HogeState {
  const HogeState();
}
...
以下のように書くとAsyncNotifierProviderが生成されます。（正確にはAutoDisposeAsyncNotifierProvider<Hoge, HogeState>）
```dart
class HogeState {
  const HogeState();
}
...
関数を使う（Provider、FutureProvider）場合、関数の引数を設定すればFamilyになります。
```dart
@riverpod
...
クラスを使う（NotifierProvider、AsyncNotifierProvider）場合、ビルドメソッドの引数を設定すればFamilyになります。
```dart
@riverpod
class Hoge extends _$Hoge {
  @override
