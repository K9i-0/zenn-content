...
maestro MCPの大きな利点は、Maestro CLIで作成したYAMLシナリオとの相乗効果です。

- **対話的操作**: MCPを使って、AIがリアルタイムでUIを操作・確認
- **再利用可能なE2Eテスト**: YAMLでシナリオを記述し、CI/CDで自動実行
- **同じSemantics ID**: 両者が同じ識別子を使うため、整合性が保たれる

機能実装時にMCPで動作確認し、その知見を元にYAMLシナリオを作成する。あるいは、既存のYAMLシナリオを`runFlow`でMCP経由で実行する。このように、MCPとYAMLシナリオを相互に活用することで、効率的な開発・テストサイクルを構築できるのではないかと考えています。
